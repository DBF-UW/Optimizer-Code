...
def velocity_parm(self, opti):
        # Velocity parameters
        self.V_turn_M1 = opti.variable(init_guess=25, lower_bound=0)
        self.V_turn_M3 = opti.variable(init_guess=25, lower_bound=0)

def mission_parm(self, opti):

        self.M1_battery = 100 # opti.variable(init_guess=constants.batteryCapacity, upper_bound=constants.batteryCapacity, lower_bound=1)
        self.M2_battery = opti.variable(init_guess=100, upper_bound=100, lower_bound=1)
        self.M3_battery = 100 # opti.variable(init_guess=constants.batteryCapacity, upper_bound=constants.batteryCapacity, lower_bound=1)
...
def airplane_parm(self, opti):
        ...
        self.n_turn_M1 = opti.variable(init_guess=3, lower_bound=1, upper_bound=constants.n_max)
...
def CD_planform(self, opti):
        ...
        self.CL_turn_M1 = self.n_turn_M1 * mass.get_weight(self.span, self.chord, 0, 0, 0, 0, self.restraint_weight, self.fuselage_area) / (0.5 * constants.rho * self.S * self.V_turn_M1**2)
        ...
        self.CD0_tail = aero.get_CD0_tail(self.Swet_ht, self.Swet_vt, self.S)
        self.CD_turn_M1 = self.skin_friction_drag + self.k * self.CL_turn_M1**2
        self.CD_turn_M3 = self.skin_friction_drag + self.k * self.CL_turn_M3**2

def powerParm(self, opti):
       
        self.power_turn_M1 = self.Drag_turn_M1 * self.V_turn_M1
...
def fuselage_parm(self, opti):
        ...
        puck_volume = (np.pi*(constants.puck_diameter/2)**2)*constants.puck_thickness/constants.PUCK_PACKING_COEFFICEINT
        ...
        self.effective_diameter = np.sqrt(self.frontal_area/np.pi)*2
